...
describe("calculerIR", () => {
  ...
  test("Revenu 20000 EUR celibataire → TMI 11%", () => {
    // Tranche 0% : 0
    // Tranche 11% : (20000 - 11601) * 0.11 = 8399 * 0.11 = 923.89
    const result = calc(20000);
    expect(result.montantIR).toBeCloseTo(923.89, 0);
    expect(result.tauxMarginal).toBe(0.11);
  });

  test("Revenu 30000 EUR celibataire → TMI 30%", () => {
    // Tranche 0% : 0
    // Tranche 11% : (29579 - 11601) * 0.11 = 17978 * 0.11 = 1977.58
    // Tranche 30% : (30000 - 29580) * 0.30 = 420 * 0.30 = 126
    // Total = 2103.58
    const result = calc(30000);
    expect(result.montantIR).toBeCloseTo(2103.58, 0);
